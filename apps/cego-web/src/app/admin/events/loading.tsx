export default function Loading() {
  return (
    <>
      ...
      <main className="page-shell mx-auto max-w-5xl px-5 pt-20 pb-16">
        <div className="space-y-4">
          <div className="h-10 w-48 animate-pulse rounded" style={{ background: "var(--color-surface-hover)" }} />
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="glass-lg rounded-2xl p-5 space-y-3">
              <div className="flex items-center gap-3">
                <div className="h-8 w-8 animate-pulse rounded-full" style={{ background: "var(--color-surface-hover)" }} />
                <div className="h-4 w-32 animate-pulse rounded" style={{ background: "var(--color-surface-hover)" }} />
              </div>
              <div className="h-4 w-2/3 animate-pulse rounded" style={{ background: "var(--color-surface-hover)" }} />
            </div>
          ))}
        </div>
      </main>
