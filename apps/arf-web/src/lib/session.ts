...
import { eq, getDb, members, type Member } from "@arf/db";

const SESSION_COOKIE_NAME = "arf_session";
...
export function isAdminTelegramId(telegramId: string): boolean {
  return (process.env.ARF_ADMIN_TELEGRAM_IDS ?? "")
    .split(/[\s,]+/)
...
function sign(value: string): string {
  ...
  if (!secret) {
    throw new Error("SESSION_SECRET is required for ARF sessions.");
  }
